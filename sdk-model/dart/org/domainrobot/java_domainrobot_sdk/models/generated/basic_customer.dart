...
/* The customers sepa mandate if payament was post with sepa. */
  ...
  BasicCustomer.fromJson(Map<String, dynamic> json) {
    if (json == null) return;
    number =
        json['number']
    ;
...
;
  ...
  static List<BasicCustomer> listFromJson(List<dynamic> json) {
    return json == null ? new List<BasicCustomer>() : json.map((value) => new BasicCustomer.fromJson(value)).toList();
  }

  static Map<String, BasicCustomer> mapFromJson(Map<String, Map<String, dynamic>> json) {
    var map = new Map<String, BasicCustomer>();
    if (json != null && json.length > 0) {
      json.forEach((String key, Map<String, dynamic> value) => map[key] = new BasicCustomer.fromJson(value));
    }
    return map;
  }
}
